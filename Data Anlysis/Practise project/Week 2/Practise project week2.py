...
def get_county_attributes(svg_file_name):
    ...
    id_strings = [path.getAttribute('id') for path in doc.getElementsByTagName('path')]
    attr_strings = [path.getAttribute('d') for path in doc.getElementsByTagName('path')]
    doc.unlink()
    table = list(zip(id_strings,attr_strings))
...
tuple1 = ('21119', 'M 425.90398,172.667 L 426.90998,172.816 z 427.64998,173.226 L 427.74298,173.371 L 427.79298,173.794 L 427.77098,174.145 L 427.95498,174.685 L 428.00898,174.74 L 428.23498,174.871 L 428.42898,174.957 L 428.23098,175.164 L 427.55398,175.394 L 426.85598,175.88 L 426.66598,176.124 L 426.46398,176.327 L 426.06698,176.642 L 425.84798,176.639 L 425.66498,176.512 L 424.04398,173.987 L 425.64298,172.487 L 425.90398,172.667')
tuple2 = ('01045', 'M 405.63598,251.83 L 409.24698,251.415 L 409.67498,255.323 L 409.71998,255.692 L 409.02198,256.291 L 408.85498,256.306 L 408.35898,256.26 L 408.07398,256.183 L 407.69998,256.094 L 407.52498,256.085 L 407.33998,256.094 L 407.08298,256.179 L 407.02098,256.233 L 406.63198,257.211 L 405.74898,257.292 L 405.19098,251.879 L 405.63598,251.83')

# Code to compute the center of a county from its boundary (as a string)

#a = [tuple([float(b) for b in x.split(',')]) for x in a.split() if not x in ['L','z','M'] ]
...
def get_boundary_coordinates(boundary_data):
    ...
    Ignores 'M', 'L, 'z'
    """
    boundary_data = boundary_data.replace('M','')
    table = [tuple([float(b) for b in x.split(',')])
             for x in boundary_data.split() if not x in ['L','z','M']]
