...
def make_dict(table, key_col):
    ...
    for row in table:
        row1 = row.copy()
        row1.pop(key_col)
        dict1[row[key_col]] = row1
    return dict1
# print (make_dict(table,1))
...
def merge_csv_files(cancer_csv_file, center_csv_file, joined_csv_file):
    ...
    """
    # state, Country name, Fips code, Population, Cancer risk, Horiz coord, Vert coords
    table_cancer = read_csv_file(cancer_csv_file)
    table_center = read_csv_file(center_csv_file)
    dict_caner = make_dict(table_cancer,2)
    dict_center = make_dict(table_center,0)
    result_table = []
    cancer_without_center =[]
    centers_without_cancer_data = []
    for row in table_cancer:
        if row[2] in dict_center:
            result_table.append(row+dict_center[row[2]])
        else:
            result_table.append(row)
            cancer_without_center.append(row)
    for row in table_center:
        if row[0] not in dict_caner:
            centers_without_cancer_data.append(row)

    print ('len table canser is ',len(table_cancer))
    print('len table center is ', len(table_center))
    print('len result table is ', len(result_table))
    print('len table of data without center is ', len(cancer_without_center))
    print('len table centers without cancer data is ', len(centers_without_cancer_data))

    # Read in both CSVtest_make_dict files
    write_csv_file(result_table,joined_csv_file)
    write_csv_file(cancer_without_center, 'cancerWithOutCenters.csv')
    write_csv_file(centers_without_cancer_data, 'centerWithOutCancer.csv')
