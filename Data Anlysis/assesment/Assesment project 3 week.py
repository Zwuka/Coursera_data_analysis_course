...
##            table.append(row)
    table = list(filter(lambda row : int(row[yearid]) == year, statistics))
    return table   
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    table = []
    for row in statistics:
        player = row[info['playerid']]
        stat = formula(info,row)
        table.append((player,stat))
    table.sort(key = lambda pair:pair[1],reverse = True)    

##    table = list(map(lambda pair1,pair2:(pair1['playerID'],
##    formula(info,pair2)),statistics,statistics)) 
   
    table.sort(key = lambda pair:pair[1],reverse = True) 
    
    return table[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    #table = list(map(lambda row : row
    dict1 = read_csv_as_nested_dict(info["masterfile"],
                                    info["playerid"],
                                    info["separator"],
                                    info["quote"])
    table = []
    for row in top_ids_and_stats:
        str1 = str('{0:.3f}'.format(row[1]))
        str2 = dict1[row[0]][info["firstname"]]
        str3 = dict1[row[0]][info["lastname"]]
        str4 = str1+' --- '+str2+' '+str3
        table.append(str4)
    return table
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    top_players = top_player_ids(info, new_stats , formula, numplayers)
    
    table = lookup_player_names(info, top_players)
